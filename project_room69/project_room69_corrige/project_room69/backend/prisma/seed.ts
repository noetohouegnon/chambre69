...
async function createProduct(filePath: string, brandId: string, categoryId: string, subcategory?: string, collection?: string) {
  ...
  // Store a RELATIVE path (not a hardcoded localhost URL).
  // The frontend combines this with VITE_API_URL to build the final image link,
  // so it works correctly both locally and once deployed online.
